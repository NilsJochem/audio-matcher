...
impl error::Error {
    pub(crate) fn expect_starts_with(data: &[u8], expect: &str) -> Result<(), Self> {
        let data = &data[0..expect.bytes().len()];
        require!(
            expect.as_bytes() == data,
            Self::MalformedData(String::from_utf8(data.to_vec()).map_or_else(
                |_| {
                    format!(
    ...
    pub(crate) fn expect_starts_with_reader(
        ...
        let mut buf = vec![0; expect.bytes().len()];
