...
impl OggPage {
    pub fn new(
        ...
        segment_table: Vec<Vec<u8>>,
    ) -> Result<Self, SegmentTooLarge> {
        SegmentTooLarge::validate(&segment_table)?;
        Ok(Self {
            header_type,
            granule_position,
    ...
    pub fn set_segment_table(
        &mut self,
        segment_table: Vec<Vec<u8>>,
    ) -> Result<(), SegmentTooLarge> {
        SegmentTooLarge::validate(&segment_table)?;
        self.segment_table = segment_table;
        Ok(())
    }
    pub fn add_segment(&mut self, segment: Vec<u8>) -> Result<(), SegmentTooLarge> {
        SegmentTooLarge::validate_new(&self.segment_table, &segment)?;
